                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      Text("Chỉnh sửa ${widget.category}", style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: Colors.white)),
                    ],
                  ),
                ),
                Expanded(
...
class _ProductInputSectionState extends State<ProductInputSection> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        const Text("Chọn sản phẩm/dịch vụ", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        const SizedBox(height: 10),
        DropdownButton<String>(
          value: selectedProduct,
          hint: const Text("Chọn sản phẩm/dịch vụ"),
          isExpanded: true,
          onChanged: (String? newValue) {
            setState(() {
              ...
              if (newValue != null) {
                ...
                widget.priceController.text = selectedPrice.toStringAsFixed(2);
                isFlexiblePrice = false; // Reset giá linh hoạt khi chọn sản phẩm mới
              }
            });
          },
          items: widget.productList.isEmpty
              ? [const DropdownMenuItem<String>(value: null, child: Text("Chưa có sản phẩm nào"))]
              : widget.productList.map((p) => DropdownMenuItem<String>(value: p["name"], child: Text(p["name"]))).toList(),
        ),
        const SizedBox(height: 20),
        CheckboxListTile(
          title: const Text("Giá linh hoạt"),
        ...
        Row(
          children: [
            ...
            Expanded(
              child: TextField(
                keyboardType: TextInputType.number,
                decoration: const InputDecoration(border: OutlineInputBorder(), hintText: "Nhập số lượng"),
            ...
            builder: (context, List<Map<String, dynamic>> history, _) {
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    child: ListTile(
                      title: Text("${transaction['name']} - ${transaction['quantity']} x ${transaction['price']} VNĐ"),
                      subtitle: Text("Tổng: ${transaction['total'].toStringAsFixed(2)} VNĐ"),
                      trailing: Row(
