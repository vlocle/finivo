...
class _EditSecondaryRevenueScreenState
  ...
  void addTransaction(
    ...
    // --- PHẦN 1: VALIDATE DỮ LIỆU ĐẦU VÀO (Tương tự) ---
    ...
    // Chú ý các key có hậu tố _Secondary
    ...
    appState.addTransactionAndUpdateState( // Giả định bạn đã tạo hàm này ở Bước 1
      ...
      _showStyledSnackBar("Lỗi khi thêm giao dịch (DTP): $e", isError: true);
    });
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                      labelText:
                      "Chi phí biến đổi/ĐV mới (Tổng - Chỉ xem)",
                      labelStyle:
                      GoogleFonts.poppins(color: AppColors.getTextSecondaryColor(context)),
                      border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(12)),
                  ...
                  inputFormatters: [
                    ...
                    TextInputFormatter.withFunction((oldValue, newValue) {
                      ...
                      final formattedText =
                      internalPriceFormatter.format(number);
            ...
            ElevatedButton(
              ...
              onPressed: () {
                ...
                if (salesTransactionId != null) {
                  final String? originalCogsSourceType =
                  updatedTransaction['cogsSourceType_Secondary'] as String?;
                  final List<dynamic>? rawCogs =
                  ...
                  if (components != null && components.isNotEmpty) {
                    for (var component in components) {
                      ...
                      if (newComponentAmount > 0) {
                        newAutoGeneratedCogs.add({
                          "name":
                          "${component['name']} (Cho DTP: ${updatedTransaction['name']})",
                    ...
                    newAutoGeneratedCogs.add({
                      "name":
                      "Giá vốn hàng bán (DTP): ${updatedTransaction['name']}",
                ...
                appState
                  ...
                  if (mounted) {
                    _showStyledSnackBar(
                        "Đã cập nhật (DTP): ${transactionToEdit['name']}. Giá vốn tự động đã được điều chỉnh.");
                  }
                }).catchError((e) {
                  if (mounted) {
                    _showStyledSnackBar("Lỗi khi cập nhật (DTP): $e",
                        isError: true);
                  }
                });
        ...
        body: FutureBuilder<List<Map<String, dynamic>>>( // [cite: 197]
          ...
          builder: (context, snapshot) { // [cite: 197]
            if (snapshot.connectionState == ConnectionState.waiting) { // [cite: 197]
              return Center(
                  child: CircularProgressIndicator(color: AppColors.chartGreen)); // [cite: 198]
            }
            if (snapshot.hasError) { // [cite: 199]
              return Center( // [cite: 199]
                  child: Text("Lỗi tải dữ liệu sản phẩm (DTP)", // [cite: 199]
                      style: GoogleFonts.poppins(
                          color: AppColors.getTextSecondaryColor(context)))); // [cite: 199, 200]
            }
                ...
                child: IndexedStack( // [cite: 201]
                  ...
                  children: [ // [cite: 202]
                    ...
                    TransactionHistorySection(
                      ...
                      _appState.secondaryRevenueTransactions, // [cite: 212] // Danh sách cho DTP
                      ...
                      onRemoveTransaction: canEditThisRevenue
                        ...
                        removeTransaction(localAppState, localTransactions, index, 'Doanh thu phụ');
                      }
                          : null,
                      appState: _appState, // [cite: 213]
                      currencyFormat: currencyFormat, // [cite: 213]
                      primaryColor: AppColors.chartGreen, // [cite: 213]
                      ...
                      child: ListTile( // [cite: 524, 525]
                        ...
                        trailing: IconButton( // [cite: 531]
                          icon: Icon(Icons.edit_note_outlined, // [cite: 532, 533]
                              color: AppColors.chartGreen.withOpacity(0.8),
                              size: 24), // [cite: 533]
                          onPressed: () { // [cite: 533]
