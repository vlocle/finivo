...
class _EditMainRevenueScreenState extends State<EditMainRevenueScreen>
  ...
  void addTransaction(AppState appState, List<Map<String, dynamic>> transactions, String? selectedProduct, double currentSelectedPriceInDropdown, bool isFlexiblePriceEnabled) {
    ...
    if (isFlexiblePriceEnabled) {
      ...
      if (priceToUse <= 0.0) {
        _showStyledSnackBar("Vui lòng nhập giá trị hợp lệ cho giá!", isError: true);
        return;
      }
    } else {
    ...
    RevenueManager.saveTransactionHistory(appState, "Doanh thu chính", transactions);
    ...
    showDialog(
      ...
      builder: (dialogContext) => GestureDetector(
        ...
        child: AlertDialog(
          ...
          title: Text("Chỉnh sửa: ${transactionToEdit['name']}", style: GoogleFonts.poppins(fontWeight: FontWeight.w600, color: _textColorPrimary)),
          content: TextField(
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                int newQuantity = int.tryParse(editQuantityController.text) ?? transactionToEdit['quantity'];
                if (newQuantity <=0) {
                  ScaffoldMessenger.of(dialogContext).showSnackBar(
                    SnackBar(content: Text("Số lượng phải lớn hơn 0", style: GoogleFonts.poppins()), backgroundColor: _accentColor, behavior: SnackBarBehavior.floating),
                  );
                  return;
                }
                transactionToEdit['quantity'] = newQuantity;
                transactionToEdit['total'] = (transactionToEdit['price'] as num? ?? 0.0) * newQuantity;
                RevenueManager.saveTransactionHistory(appState, "Doanh thu chính", transactions);
        ...
        body: FutureBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasError) {
              return Center(child: Text("Lỗi tải dữ liệu sản phẩm", style: GoogleFonts.poppins(color: _textColorSecondary)));
            }
            ...
            return ScaleTransition(
                ...
                child: IndexedStack(
                  ...
                  children: [
                    ProductInputSection(
                      key: const ValueKey('productInput'),
                      productList: productList,
                      quantityController: quantityController,
                      priceController: priceController,
                      onAddTransaction: (selectedProduct, selectedPrice, isFlexiblePrice) {
                        addTransaction(appState, appState.mainRevenueTransactions.value, selectedProduct, selectedPrice, isFlexiblePrice);
                      },
                      appState: appState,
...
class _ProductInputSectionState extends State<ProductInputSection> {
  ...
  void _updatePriceControllerBasedOnSelection() {
    if (!isFlexiblePriceEnabled && selectedProduct != null) {
      final product = widget.productList.firstWhere((p) => p["name"] == selectedProduct, orElse: () => {"price": 0.0});
  ...
  Widget build(BuildContext context) {
    ...
    const Color textColorSecondary = _EditMainRevenueScreenState._textColorSecondary;
      ...
      child: Column(
        ...
        children: [
          Card(
            ...
            color: _EditMainRevenueScreenState._cardBackgroundColor,
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    "Thêm giao dịch mới",
                    style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.w700, color: primaryColor),
                  ),
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: const Icon(Icons.sell_outlined, color: primaryColor, size: 22),
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                      filled: true,
                    ...
                    items: widget.productList.isEmpty
                      ...
                      const DropdownMenuItem<String>(
                        value: null,
                        child: Text("Chưa có sản phẩm nào", style: TextStyle(fontStyle: FontStyle.italic)),
                      )
                    ]
                        : widget.productList
                        .map((p) => DropdownMenuItem<String>(
                      value: p["name"],
                      child: Text(p["name"], overflow: TextOverflow.ellipsis, maxLines: 1, style: GoogleFonts.poppins(color: textColorPrimary)),
                    ))
                        .toList(),
                    onChanged: (String? newValue) {
                      setState(() {
                        ...
                        if (newValue != null) {
                          final productData = widget.productList.firstWhere((p) => p["name"] == newValue, orElse: () => {"price": 0.0});
                          selectedPriceFromDropdown = (productData["price"] as num? ?? 0.0).toDouble();
                          if (!isFlexiblePriceEnabled) {
                            widget.priceController.text = _priceInputFormatter.format(selectedPriceFromDropdown);
                          }
                        } else {
                  ...
                  SwitchListTile.adaptive(
                    title: Text(
                      "Giá linh hoạt",
                      style: GoogleFonts.poppins(fontSize: 16, color: textColorPrimary, fontWeight: FontWeight.w500),
                    ),
                    ...
                    onChanged: (bool value) {
                      setState(() {
                        ...
                        if (!isFlexiblePriceEnabled) {
                          ...
                          if (selectedProduct == null) widget.priceController.text = _priceInputFormatter.format(0);
                          else widget.priceController.text = _priceInputFormatter.format(selectedPriceFromDropdown);
                          _priceFocusNode.requestFocus();
                        }
                      });
                    },
                    contentPadding: EdgeInsets.zero,
                    secondary: Icon(isFlexiblePriceEnabled ? Icons.edit_attributes_outlined : Icons.attach_money_outlined, color: primaryColor, size: 22),
                  ),
                  _buildModernTextField(
                      ...
                      inputFormatters: [
                        ...
                        TextInputFormatter.withFunction(
                            ...
                            // Remove existing formatting before parsing
                            final String plainNumberText = newValue.text.replaceAll('.', '').replaceAll(',', '');
                            final number = int.tryParse(plainNumberText);
                            if (number == null) return oldValue; // Keep old value if parsing fails
                            final formattedText = _priceInputFormatter.format(number);
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          "TỔNG TIỀN ƯỚC TÍNH:",
                          style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600, color: primaryColor.withOpacity(0.8), letterSpacing: 0.5),
                        ),
                        SizedBox(height: 4),
                        Text(
                          widget.currencyFormat.format((double.tryParse(widget.priceController.text.replaceAll('.', '').replaceAll(',', '')) ?? (isFlexiblePriceEnabled ? 0.0 : selectedPriceFromDropdown)) * (int.tryParse(widget.quantityController.text) ?? 1)),
                          style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.w700, color: primaryColor),
                        ),
                      ],
                    ),
                  ),
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () => widget.onAddTransaction(selectedProduct, selectedPriceFromDropdown, isFlexiblePriceEnabled),
                    child: Text(
                      "Thêm giao dịch",
  ...
  Widget _buildModernTextField({
    ...
    const Color secondaryColor = _EditMainRevenueScreenState._secondaryColor;
    const Color textColorSecondary = _EditMainRevenueScreenState._textColorSecondary;
    const Color cardBackgroundColorFromParent = _EditMainRevenueScreenState._cardBackgroundColor;
    ...
    return TextField(
      ...
      focusNode: focusNode,
      onChanged: onChanged,
      style: GoogleFonts.poppins(color: _EditMainRevenueScreenState._textColorPrimary, fontWeight: FontWeight.w500, fontSize: 16),
      decoration: InputDecoration(
        labelText: labelText,
        labelStyle: GoogleFonts.poppins(color: textColorSecondary),
        ...
        filled: true,
        fillColor: enabled ? cardBackgroundColorFromParent : secondaryColor.withOpacity(0.5),
        contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
        counterText: "",
      ),
      maxLines: 1,
                ...
                itemBuilder: (context, index) {
                  ...
                  return Dismissible(
                    ...
                    background: Container(
                      color: _EditMainRevenueScreenState._accentColor.withOpacity(0.8),
                    ...
                    onDismissed: (direction) {
                      onRemoveTransaction(appState, transactionsNotifier.value, history.length - 1 - index);
                    },
                    child: Card(
                      elevation: 1.5,
                        ...
                        leading: CircleAvatar(
                          ...
                          child: Text(
                            transaction['name'] != null && (transaction['name'] as String).isNotEmpty
                                ...
                                : "?",
                            style: GoogleFonts.poppins(color: primaryColor, fontWeight: FontWeight.w600, fontSize: 18),
                          ),
                        ),
                        title: Text(
                          transaction['name']?.toString() ?? 'N/A',
                          style: GoogleFonts.poppins(fontSize: 15.5, fontWeight: FontWeight.w600, color: textColorPrimary),
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            Text(
                              ...
                              style: GoogleFonts.poppins(fontSize: 12.5, color: textColorSecondary),
                            ),
                            Text(
                              "Tổng: ${currencyFormat.format(transaction['total'] ?? 0.0)}",
                              style: GoogleFonts.poppins(fontSize: 13.0, color: primaryColor, fontWeight: FontWeight.w500),
                            ),
                          ],
                        ),
                        trailing: IconButton(
