                      ...
                      Row(
                        children: [
                          ...
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text("Chi phí cố định", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: Colors.white)),
                              Container(
                                ...
                                child: Card(
                                  ...
                                  child: Padding(
                                    ...
                                    child: Row(
                                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                      children: [
                                        const Text('Tổng chi phí cố định', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                                        Text(currencyFormat.format(appState.fixedExpense), style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF1976D2))),
                                      ],
                                    ),
                                  ),
                                ),
                                      ...
                                      builder: (context, List<Map<String, dynamic>> expenses, _) {
                                        return Card(
                                          ...
                                          child: expenses.isEmpty
                                              ? const Center(child: Text("Chưa có chi phí cố định", style: TextStyle(fontSize: 16, color: Colors.grey)))
                                              : ListView.builder(
  ...
  void showEditAmountDialog(int index, AppState appState) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        title: Text("Chỉnh sửa số tiền - ${appState.fixedExpenseList.value[index]['name']}"),
        content: TextField(
          controller: amountController,
          keyboardType: TextInputType.number,
          decoration: InputDecoration(labelText: "Nhập số tiền", border: OutlineInputBorder(borderRadius: BorderRadius.circular(8))),
        ),
        actions: [
  ...
  Future<void> deleteExpenseItem(int index, AppState appState) async {
    bool? confirm = await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("Bạn có chắc muốn xóa '${appState.fixedExpenseList.value[index]['name']}' không?"),
        actions: [
