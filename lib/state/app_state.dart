...
// Hàm phụ để so sánh hai danh sách
  ...
  Future<void> payForVariableExpenseAndUpdateState({
    ...
    final String expenseId = expenseToPay['id'];
    final double amountToPay = (expenseToPay['amount'] as num).toDouble();
    ...
    final txIndex = list.indexWhere((t) => t['id'] == expenseId);
    if (txIndex != -1) {
      final newList = List<Map<String, dynamic>>.from(list);
      newList[txIndex] = fullyUpdatedExpense;
      variableExpenseList.value = newList;
    }

    // Tạo giao dịch điều chỉnh ví để ghi lại lịch sử
    ...
    if (walletIndex != -1) {
      ...
      updatedWallet['balance'] = currentBalance - amountToPay;
    ...
    try {
      // Lưu lại toàn bộ danh sách chi phí biến đổi của ngày
      await ExpenseManager.saveVariableExpenses(this, variableExpenseList.value, batch: batch);

      // Thêm giao dịch điều chỉnh ví vào daily_data
      final DateTime recordDate = DateTime.parse(expenseToPay['date'] as String);
      final dateKey = DateFormat('yyyy-MM-dd').format(recordDate);
      final dailyDataRef = firestore.collection('users').doc(activeUserId).collection('daily_data').doc(getKey(dateKey));
      ...
      batch.update(walletRef, {'balance': FieldValue.increment(-amountToPay)});

      await batch.commit();
    } catch (e) {
      print("Lỗi khi thực chi cho chi phí: $e");
      throw e;
    }
  }
  ...
  Future<void> addTransactionAndUpdateState({
    ...
    if (isCashReceived && walletId != null) {
      ...
      newSalesTransaction['paymentStatus'] = 'unpaid';
    }
    ...
    List<Map<String, dynamic>> processedCogs = []; // Danh sách COGS đã xử lý

    if (autoGeneratedCogs.isNotEmpty) {
      processedCogs = autoGeneratedCogs.map((cog) {
        final newCog = Map<String, dynamic>.from(cog);
        newCog['id'] = Uuid().v4();
        final amount = (newCog['amount'] as num?)?.toDouble() ?? 0.0;
        totalCogsAmount += amount;

        if (isCashSpent && spendingWalletId != null) {
          newCog['paymentStatus'] = 'paid';
          newCog['walletId'] = spendingWalletId;
          newCog['paymentDate'] = DateTime.now().toIso8601String();

          // === BƯỚC SỬA LỖI QUAN TRỌNG ===
          // Tạo một giao dịch "Điều chỉnh Ví" để ghi lại dấu vết trong lịch sử
          final adjustmentTransaction = {
            'id': Uuid().v4(),
            'name': 'Thanh toán: ${newCog['name']}', // Tên chi phí gốc
            'category': 'Điều chỉnh Ví',
            ...
            'sourceSalesTransactionId': newSalesTransaction['id'],
          };
          walletAdjustments.value.add(adjustmentTransaction);
          // ==============================

        } else {
          newCog['paymentStatus'] = 'unpaid';
        }
        return newCog;
      }).toList();

      variableExpenseList.value.addAll(processedCogs);
      variableExpenseList.value = List.from(variableExpenseList.value);
      walletAdjustments.value = List.from(walletAdjustments.value); // Trigger update
    }

    // 2. TÍNH TOÁN LẠI TỔNG VÀ SỐ DƯ VÍ
    // ... (phần code tính toán và cập nhật số dư ví giữ nguyên)
    ...
    if (isCashReceived && walletId != null) {
      ...
      if (index != -1) {
        final currentBalance = (wallets.value[index]['balance'] as num?)?.toDouble() ?? 0.0;
        final transactionAmount = (newSalesTransaction['total'] as num?)?.toDouble() ?? 0.0;
        wallets.value[index]['balance'] = currentBalance + transactionAmount;
      }
    }

    if (isCashSpent && spendingWalletId != null) {
      final index = wallets.value.indexWhere((w) => w['id'] == spendingWalletId);
      if (index != -1) {
        final currentBalance = (wallets.value[index]['balance'] as num?)?.toDouble() ?? 0.0;
        wallets.value[index]['balance'] = currentBalance - totalCogsAmount;
      }
    }

    if(isCashReceived || isCashSpent) {
      wallets.value = List.from(wallets.value);
    }
    ...
    // Hàm _saveAllDailyDataToFirestore giờ sẽ lưu cả walletAdjustments mới
    await _saveAllDailyDataToFirestore(batch: batch);
    await ExpenseManager.saveVariableExpenses(this, variableExpenseList.value, batch: batch);

    if (isCashReceived && walletId != null) {
      final walletRef = firestore.collection('users').doc(activeUserId).collection('wallets').doc(walletId);
      batch.update(walletRef, {'balance': FieldValue.increment((newSalesTransaction['total'] as num).toDouble())});
    }

    if (isCashSpent && spendingWalletId != null) {
      final walletRef = firestore.collection('users').doc(activeUserId).collection('wallets').doc(spendingWalletId);
      batch.update(walletRef, {'balance': FieldValue.increment(-totalCogsAmount)});
    }
  ...
  Future<void> editTransactionAndUpdateState({
    ...
    // BƯỚC 1: TÍNH TOÁN CÁC CHÊNH LỆCH VÀ XÁC ĐỊNH TRẠNG THÁI GỐC
    final double oldRevenueTotal = (originalTransaction['total'] as num?)?.toDouble() ?? 0.0;
    final double newRevenueTotal = (updatedTransaction['total'] as num?)?.toDouble() ?? 0.0;
    final double revenueDelta = newRevenueTotal - oldRevenueTotal;
    final String? revenueWalletId = originalTransaction['walletId'] as String?;
    final bool revenueWasPaid = originalTransaction['paymentStatus'] == 'paid';

    final double oldCogsTotal = (originalTransaction['totalVariableCost'] as num?)?.toDouble() ?? 0.0;
    final double newCogsTotal = (updatedTransaction['totalVariableCost'] as num?)?.toDouble() ?? 0.0;
    final double cogsDelta = newCogsTotal - oldCogsTotal;

    // Tìm các chi phí COGS cũ để xác định trạng thái thanh toán thực tế của chúng
    final oldCogs = variableExpenseList.value.where((exp) => exp['sourceSalesTransactionId'] == transactionId).toList();
    final bool originalCogsWerePaid = oldCogs.isNotEmpty && oldCogs.first['paymentStatus'] == 'paid';
    final String? cogsWalletId = oldCogs.isNotEmpty ? oldCogs.first['walletId'] as String? : null;
    final String? cogsPaymentDate = oldCogs.isNotEmpty ? oldCogs.first['paymentDate'] as String? : null;

    // BƯỚC 2: CẬP NHẬT SỐ DƯ VÍ (LOGIC ĐÃ TÁCH BIỆT)
    // Cập nhật ví cho phần doanh thu (nếu doanh thu đã được thanh toán)
    if (revenueWasPaid && revenueWalletId != null && revenueDelta != 0) {
      final walletRef = firestore.collection('users').doc(activeUserId).collection('wallets').doc(revenueWalletId);
      batch.update(walletRef, {'balance': FieldValue.increment(revenueDelta)});
      final walletIndex = wallets.value.indexWhere((w) => w['id'] == revenueWalletId);
      if (walletIndex != -1) wallets.value[walletIndex]['balance'] += revenueDelta;
    }

    // Cập nhật ví cho phần chi phí (chỉ khi chi phí gốc đã được thanh toán)
    if (originalCogsWerePaid && cogsWalletId != null && cogsDelta != 0) {
      final walletRef = firestore.collection('users').doc(activeUserId).collection('wallets').doc(cogsWalletId);
      batch.update(walletRef, {'balance': FieldValue.increment(-cogsDelta)});
      final walletIndex = wallets.value.indexWhere((w) => w['id'] == cogsWalletId);
      if (walletIndex != -1) wallets.value[walletIndex]['balance'] -= cogsDelta;
    }

    // BƯỚC 3: CẬP NHẬT DANH SÁCH GIAO DỊCH
    // Cập nhật danh sách doanh thu local
    ...
    // Cập nhật danh sách chi phí biến đổi local
    variableExpenseList.value.removeWhere((expense) => expense['sourceSalesTransactionId'] == transactionId);

    // Gán lại trạng thái thanh toán đúng cho COGS mới
    final List<Map<String, dynamic>> processedNewCogs = newCogsTransactions.map((cog) {
      final newCogWithId = Map<String, dynamic>.from(cog);
      newCogWithId['id'] ??= Uuid().v4();

      // Sử dụng trạng thái của COGS gốc để quyết định trạng thái cho COGS mới
      if (originalCogsWerePaid && cogsWalletId != null) {
        newCogWithId['paymentStatus'] = 'paid';
        newCogWithId['walletId'] = cogsWalletId;
        newCogWithId['paymentDate'] = cogsPaymentDate ?? DateTime.now().toIso8601String();
      } else {
        newCogWithId['paymentStatus'] = 'unpaid';
      }
      return newCogWithId;
    }).toList();
    variableExpenseList.value.addAll(processedNewCogs);

    // Cập nhật bản ghi thanh toán (Wallet Adjustment) local (chỉ khi COGS đã được trả)
    if (originalCogsWerePaid) {
      final adjIndex = walletAdjustments.value.indexWhere((adj) => adj['sourceSalesTransactionId'] == transactionId);
      if (adjIndex != -1) {
        walletAdjustments.value[adjIndex]['total'] = -newCogsTotal;
        walletAdjustments.value[adjIndex]['name'] = "Thanh toán: ${updatedTransaction['name']}";
      }
    }

    // Trigger update cho các list local và UI
    ...
    // BƯỚC 4: LƯU TẤT CẢ THAY ĐỔI LÊN SERVER
    await _saveAllDailyDataToFirestore(batch: batch);
    await ExpenseManager.saveVariableExpenses(this, variableExpenseList.value, batch: batch);
    await batch.commit();
  }
...
// Hàm thêm giao dịch "Doanh thu khác"
  ...
  // Thay thế toàn bộ hàm này trong file appstate.docx
  Future<void> deleteTransactionAndUpdateAll({
    ...
    // <<< SỬA LỖI: Theo dõi các thay đổi để cập nhật local >>>
    double revenueToRevert = 0.0;
    double cogsToRevert = 0.0;
    String? walletIdToUpdate = transactionToRemove['walletId'] as String?;
    final cogsToRemoveFromLocal = variableExpenseList.value.where((exp) => exp['sourceSalesTransactionId'] == transactionId).toList();
    // <<< KẾT THÚC SỬA LỖI >>>

    final DateTime transactionDate = DateTime.parse(transactionToRemove['date'] as String);
    ...
    try {
      ...
      await firestore.runTransaction((transaction) async {
        // (Phần logic transaction trên Firestore giữ nguyên như cũ)
        final dailyDataDocRef = firestore.collection('users').doc(activeUserId).collection('daily_data').doc(getKey(dateKey));
        final variableExpenseDocId = getKey('variableTransactionHistory_$dateKey');
        final variableExpenseRef = firestore.collection('users').doc(activeUserId).collection('expenses').doc('variable').collection('daily').doc(variableExpenseDocId);
        final fixedExpenseDocId = getKey('fixedExpenseList_$dateKey');
        final fixedExpenseRef = firestore.collection('users').doc(activeUserId).collection('expenses').doc('fixed').collection('daily').doc(fixedExpenseDocId);
        final dailySnapshot = await transaction.get(dailyDataDocRef);
        final variableSnapshot = await transaction.get(variableExpenseRef);
        final fixedSnapshot = await transaction.get(fixedExpenseRef);
        if (!dailySnapshot.exists) { return; }
        final dailyData = dailySnapshot.data() as Map<String, dynamic>? ?? {};

        final bool revenueWasPaid = transactionToRemove['paymentStatus'] == 'paid';
        if (revenueWasPaid && walletIdToUpdate != null) {
          revenueToRevert = (transactionToRemove['total'] as num?)?.toDouble() ?? 0.0;
          final walletRef = firestore.collection('users').doc(activeUserId).collection('wallets').doc(walletIdToUpdate);
          transaction.update(walletRef, {'balance': FieldValue.increment(-revenueToRevert)});
        }

        List<Map<String, dynamic>> cogsList = [];
        if (variableSnapshot.exists) {
          final variableData = variableSnapshot.data() as Map<String, dynamic>? ?? {};
          cogsList = List.from(variableData['variableExpenses'] ?? []);
          final cogsToRemove = cogsList.where((exp) => exp['sourceSalesTransactionId'] == transactionId).toList();
          for (final cog in cogsToRemove) {
            if (cog['paymentStatus'] == 'paid' && cog['walletId'] != null) {
              final double amount = (cog['amount'] as num?)?.toDouble() ?? 0.0;
              cogsToRevert += amount;
              final walletRef = firestore.collection('users').doc(activeUserId).collection('wallets').doc(cog['walletId']);
              transaction.update(walletRef, {'balance': FieldValue.increment(amount)});
            }
          }
        }

        // (Phần logic xóa bản ghi và tính toán lại trên Firestore giữ nguyên)
        List<Map<String, dynamic>> adjustments = List.from(dailyData['walletAdjustments'] ?? []);
        adjustments.removeWhere((adj) => adj['sourceSalesTransactionId'] == transactionId);
        dailyData['walletAdjustments'] = adjustments;
        final String revenueListKey;
        if (category == 'Doanh thu chính') revenueListKey = 'mainRevenueTransactions';
        else if (category == 'Doanh thu phụ') revenueListKey = 'secondaryRevenueTransactions';
        else revenueListKey = 'otherRevenueTransactions';
        List<Map<String, dynamic>> revenueList = List.from(dailyData[revenueListKey] ?? []);
        revenueList.removeWhere((t) => t['id'] == transactionId);
        dailyData[revenueListKey] = revenueList;
        cogsList.removeWhere((exp) => exp['sourceSalesTransactionId'] == transactionId);
        double newVariableTotal = cogsList.fold(0.0, (sum, item) => sum + (item['amount'] as num? ?? 0.0));
        if (variableSnapshot.exists) {
          transaction.update(variableExpenseRef, {'variableExpenses': cogsList, 'total': newVariableTotal});
        }
        double newMainRevenue = (dailyData['mainRevenueTransactions'] as List<dynamic>? ?? []).fold(0.0, (sum, e) => sum + ((e as Map)['total']?.toDouble() ?? 0.0));
        double newSecondaryRevenue = (dailyData['secondaryRevenueTransactions'] as List<dynamic>? ?? []).fold(0.0, (sum, e) => sum + ((e as Map)['total']?.toDouble() ?? 0.0));
        double newOtherRevenue = (dailyData['otherRevenueTransactions'] as List<dynamic>? ?? []).fold(0.0, (sum, e) => sum + ((e as Map)['total']?.toDouble() ?? 0.0));
        double newTotalRevenue = newMainRevenue + newSecondaryRevenue + newOtherRevenue;
        double fixedTotal = 0.0;
        if(fixedSnapshot.exists) {
          final fixedData = fixedSnapshot.data() as Map<String, dynamic>? ?? {};
          fixedTotal = (fixedData['total'] as num?)?.toDouble() ?? 0.0;
        }
        double newProfit = newTotalRevenue - (newVariableTotal + fixedTotal);
        double newProfitMargin = newTotalRevenue > 0 ? (newProfit / newTotalRevenue) * 100 : 0;
        dailyData['mainRevenue'] = newMainRevenue;
        dailyData['secondaryRevenue'] = newSecondaryRevenue;
        dailyData['otherRevenue'] = newOtherRevenue;
        dailyData['totalRevenue'] = newTotalRevenue;
        dailyData['profit'] = newProfit;
        dailyData['profitMargin'] = newProfitMargin;
        transaction.set(dailyDataDocRef, dailyData);
      });

      // === PHẦN 2: CẬP NHẬT TRẠNG THÁI LOCAL NGAY LẬP TỨC ===
      // Xóa khỏi các danh sách giao dịch
      mainRevenueTransactions.value.removeWhere((t) => t['id'] == transactionId);
      secondaryRevenueTransactions.value.removeWhere((t) => t['id'] == transactionId);
      otherRevenueTransactions.value.removeWhere((t) => t['id'] == transactionId);
      variableExpenseList.value.removeWhere((e) => e['sourceSalesTransactionId'] == transactionId);
      walletAdjustments.value.removeWhere((adj) => adj['sourceSalesTransactionId'] == transactionId);

      // Cập nhật số dư ví local
      if (walletIdToUpdate != null) {
        final walletIndex = wallets.value.indexWhere((w) => w['id'] == walletIdToUpdate);
        if (walletIndex != -1) {
          wallets.value[walletIndex]['balance'] -= revenueToRevert;
          wallets.value[walletIndex]['balance'] += cogsToRevert;
        }
      }

      // Tính toán lại các tổng cục bộ
      mainRevenue = mainRevenueTransactions.value.fold(0.0, (sum, t) => sum + ((t['total'] as num?)?.toDouble() ?? 0.0));
      secondaryRevenue = secondaryRevenueTransactions.value.fold(0.0, (sum, t) => sum + ((t['total'] as num?)?.toDouble() ?? 0.0));
      otherRevenue = otherRevenueTransactions.value.fold(0.0, (sum, t) => sum + ((t['total'] as num?)?.toDouble() ?? 0.0));
      variableExpense = variableExpenseList.value.fold(0.0, (sum, e) => sum + ((e['amount'] as num?)?.toDouble() ?? 0.0));

    // Trigger cập nhật giao diện
    _updateProfitAndRelatedListenables();
    mainRevenueTransactions.value = List.from(mainRevenueTransactions.value);
    variableExpenseList.value = List.from(variableExpenseList.value);
    walletAdjustments.value = List.from(walletAdjustments.value);
    ...
    print("Lỗi trong hàm deleteTransactionAndUpdateAll: $e");
    setSelectedDate(selectedDate); // Nếu lỗi, làm mới lại từ server cho chắc chắn
    throw e;
    }
  }
...
// Hàm xóa giao dịch "Doanh thu khác"
  ...
  Future<void> _saveAllDailyDataToFirestore({WriteBatch? batch}) async { // THAY ĐỔI 1: Thêm tham số tùy chọn
    ...
    try {
      ...
      // >> LOG BẮT ĐẦU
      print("\n--- [LOG LƯU] Chuẩn bị lưu daily_data ---");
      print("[LOG LƯU] Đường dẫn: $docPath");
      print("[LOG LƯU] Dữ liệu walletAdjustments sắp lưu: ${walletAdjustments.value}");
      // >> LOG KẾT THÚC
      ...
      if (!isExternalBatch) {
        ...
        print("[LOG LƯU] Đã commit batch nội bộ thành công.");
  ...
  Future<void> deleteWalletAndAssociatedData({
    ...
    if (wasDefault && currentWallets.isNotEmpty) {
      ...
      currentWallets[0] = {...currentWallets[0], 'isDefault': true};
    }
    wallets.value = currentWallets;

    // BẮT ĐẦU QUY TRÌNH NỀN
    final batch = firestore.batch();

    // 1. Xử lý gán ví mặc định mới trên server
    if (wasDefault && currentWallets.isNotEmpty) {
    ...
    // 2. Đọc tất cả dữ liệu liên quan MỘT LẦN DUY NHẤT
    final dailyDataSnapshot = await firestore.collection('users').doc(activeUserId).collection('daily_data').get();
    final variableExpenseSnapshot = await firestore.collection('users').doc(activeUserId).collection('expenses').doc('variable').collection('daily').get();
    final fixedExpenseSnapshot = await firestore.collection('users').doc(activeUserId).collection('expenses').doc('fixed').collection('daily').get();

    // Tạo map để tra cứu dữ liệu hiệu quả
    final variableDataMap = {for (var doc in variableExpenseSnapshot.docs) doc.id: doc.data()};
    final fixedDataMap = {for (var doc in fixedExpenseSnapshot.docs) doc.id: doc.data()};
    final Set<String> revenueIdsToDelete = {};

    // 3. Xử lý dọn dẹp và tính toán lại cho từng ngày
    for (final dailyDoc in dailyDataSnapshot.docs) {
      bool needsRecalculation = false;
      final originalDailyData = dailyDoc.data();
      final Map<String, dynamic> updatedDailyData = Map.from(originalDailyData);

      // Dọn dẹp các mảng giao dịch
      final transactionFields = ['mainRevenueTransactions', 'secondaryRevenueTransactions', 'otherRevenueTransactions', 'walletAdjustments'];
      for (final field in transactionFields) {
        final List<Map<String, dynamic>> originalList = List.from(originalDailyData[field] ?? []);
        if (originalList.isNotEmpty) {
          originalList.where((tx) => tx['walletId'] == walletIdToDelete).forEach((tx) {
            if (tx['id'] != null) revenueIdsToDelete.add(tx['id']);
          });

          final List<Map<String, dynamic>> filteredList = originalList.where((tx) => tx['walletId'] != walletIdToDelete).toList();
          if (originalList.length != filteredList.length) {
            needsRecalculation = true;
            updatedDailyData[field] = filteredList;
          }
        }
      }

      if (needsRecalculation) {
        // ÉP KIỂU DỮ LIỆU để khắc phục lỗi 'operator []'
        final dailyData = updatedDailyData as Map<String, dynamic>;
        final dateKey = dailyDoc.id.substring(dailyDoc.id.indexOf('_') + 1);

        // Lấy dữ liệu chi phí tương ứng của ngày này
        final variableDocId = getKey('variableTransactionHistory_$dateKey');
        final fixedDocId = getKey('fixedExpenseList_$dateKey');
        final variableData = variableDataMap[variableDocId];
        final fixedData = fixedDataMap[fixedDocId];

        // Dọn dẹp chi phí biến đổi liên quan
        List<Map<String, dynamic>> currentVarList = List.from(variableData?['variableExpenses'] ?? []);
        int originalVarCount = currentVarList.length;
        currentVarList.removeWhere((tx) => revenueIdsToDelete.contains(tx['sourceSalesTransactionId']));

        // Tính toán lại tất cả các số tổng
        double newVarTotal = currentVarList.fold(0.0, (sum, e) => sum + (e['amount']?.toDouble() ?? 0.0));
        double newMain = (List.from(dailyData['mainRevenueTransactions'] ?? [])).fold(0.0, (sum, e) => sum + (e['total']?.toDouble() ?? 0.0));
        double newSecondary = (List.from(dailyData['secondaryRevenueTransactions'] ?? [])).fold(0.0, (sum, e) => sum + (e['total']?.toDouble() ?? 0.0));
        double newOther = (List.from(dailyData['otherRevenueTransactions'] ?? [])).fold(0.0, (sum, e) => sum + (e['total']?.toDouble() ?? 0.0));
        double newTotalRevenue = newMain + newSecondary + newOther;
        double fixedTotal = (fixedData?['total'] as num?)?.toDouble() ?? 0.0;
        double newProfit = newTotalRevenue - (newVarTotal + fixedTotal);
        double newProfitMargin = newTotalRevenue > 0 ? (newProfit / newTotalRevenue) * 100 : 0;

        // Cập nhật tài liệu daily_data vào batch
        batch.update(dailyDoc.reference, {
          ...updatedDailyData, // Các mảng đã được lọc
          'mainRevenue': newMain,
          'secondaryRevenue': newSecondary,
          'otherRevenue': newOther,
        ...
        // Cập nhật tài liệu variable_expense vào batch nếu có thay đổi
        if (currentVarList.length != originalVarCount) {
          // TẠO LẠI THAM CHIẾU để khắc phục lỗi 'Undefined name'
          final variableDocRef = firestore.collection('users').doc(activeUserId).collection('expenses').doc('variable').collection('daily').doc(variableDocId);
          batch.update(variableDocRef, {'variableExpenses': currentVarList, 'total': newVarTotal});
        }
      }
    }

    // 4. Xóa tài liệu của ví
    final walletRef = firestore.collection('users').doc(activeUserId).collection('wallets').doc(walletIdToDelete);
    batch.delete(walletRef);

    // 5. Thực thi tất cả các thao tác
    await batch.commit();

    // 6. Đồng bộ lại trạng thái ứng dụng bằng cách "Làm mới" mạnh mẽ
    final currentSelectedDate = _selectedDate;
    setSelectedDate(currentSelectedDate);
    notifyListeners(); // Thêm notifyListeners để đảm bảo các widget không dùng ValueNotifier cũng được cập nhật
  }
  ...
  Future<void> payForVariableExpenseGroupAndUpdateState({
    required List<Map<String, dynamic>> expensesToPay,
    required String walletId,
    ...
    final adjustmentTransaction = {
      'id': Uuid().v4(),
      'name': 'Thanh toán nhóm: ${firstExpense['name']}',
      'category': 'Điều chỉnh Ví',
      'walletId': walletId,
      'total': -totalAmount,
      'date': DateTime.now().toIso8601String(),
      ...
      'sourceSalesTransactionId': firstExpense['sourceSalesTransactionId'],
      'sourceRevenueName': groupName.replaceFirst('Giá vốn cho: ', ''),
      'adjustmentType': 'cogs_payment',
    };
    walletAdjustments.value.add(adjustmentTransaction);
    ...
    final walletIndex = wallets.value.indexWhere((w) => w['id'] == walletId);
    if (walletIndex != -1) {
      wallets.value[walletIndex]['balance'] -= totalAmount;
    }
    ...
    try {
      ...
      // Thêm giao dịch điều chỉnh ví
      final DateTime recordDate = DateTime.parse(expensesToPay.first['date'] as String);
      final dateKey = DateFormat('yyyy-MM-dd').format(recordDate);
      final dailyDataRef = firestore.collection('users').doc(activeUserId).collection('daily_data').doc(getKey(dateKey));
      batch.set(dailyDataRef, {
        'walletAdjustments': FieldValue.arrayUnion([adjustmentTransaction])
      ...
      // Cập nhật số dư ví
      final walletRef = firestore.collection('users').doc(activeUserId).collection('wallets').doc(walletId);
      batch.update(walletRef, {'balance': FieldValue.increment(-totalAmount)});

      await batch.commit();
    } catch (e) {
      print("Lỗi khi thực chi cho nhóm chi phí: $e");
      throw e;
    }
  }
